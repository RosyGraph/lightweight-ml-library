...
def build_features(dataset, fix_unknown=False, test=False) -> Tuple[pd.DataFrame, dict, set]:
    if dataset == "car":
        return build_car_features(test)
    if dataset == "bank":
        return build_bank_features_majority_complete(test) if fix_unknown else build_bank_features(test)
...
def build_bank_features_majority_complete(test=False):
    path_to_columns = os.path.join("data", "bank", "columns.txt")
    columns = []
    attributes = dict()
    with open(path_to_columns, "r") as f:
    ...
    data_filename = "test.csv" if test else "train.csv"
    path_to_data = os.path.join("data", "bank", data_filename)
    df: pd.DataFrame = pd.read_csv(path_to_data, names=columns)
    for column in columns:
        mode = df[column].value_counts()
        new = mode.index[0]
        if new == "unknown":
            new = mode.index[1]
        df[column].replace("unknown", new, inplace=True)
    for numeric_column_name in ("age", "balance", "day", "duration", "campaign", "pdays", "previous"):
        median = df[numeric_column_name].median()
        names = [f"<{median}", f"{median}+"]
        bins = [-np.inf, median, np.inf]
...
if __name__ == "__main__":
    df, *_ = build_bank_features_majority_complete()
    print(df)
